...
describe('Parser character class integration', () => {
    let parser: Parser;
    ...
    it('should be able to parse a regex pattern containing a negated character class with range and special chars', () => {
        const ast = parser.parse('before[^]c-dfg-]after');

        expect(ast.getPattern()).to.equal('before[^]c-dfg-]after');
        expect(ast.getParsingAst()).to.deep.equal({
            'name': 'N_PATTERN',
            'components': [
                {
